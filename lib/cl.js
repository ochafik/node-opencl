...
function withEvents(events, f) {
    var pEvents = pointerToPointers(events.map(function(event) { return event.event }));
    var pEvent = new SizeTArray(1).buffer
    check(f(events.length, pEvents, pEvent))
...
CLCommandQueue.prototype.enqueueTask = function(kernel, num_events_in_wait_list, event_wait_list) {
    return withEvents(function(numEventInWaitList, pEventWaitList, pEvent) {
        return opencl.clEnqueueTask(this.commandQueue, kernel, numEventInWaitList, pEventWaitList, pEvent);
    });
};

CLCommandQueue.prototype.flush = function() {
    check(opencl.clFlush(this.commandQueue));
};

CLCommandQueue.prototype.finish = function() {
    check(opencl.clFinish(this.commandQueue));
};

CLCommandQueue.prototype.enqueueNDRangeKernel = function(kernel, globalOffsets, globalSizes, localSizes) {
    var dims = globalSizes.length
    return withEvents(function(numEventInWaitList, pEventWaitList, pEvent) {
        return opencl.clEnqueueNDRangeKernel(this.commandQueue, kernel, dims, globalOffsets, globalSizes, localSizes, numEventInWaitList, pEventWaitList, pEvent);
    });
};
...
CLProgram.prototype.createKernel = function(name) {
    ...
    return withCheck(function(pErr) {
        return new CLKernel(opencl.clCreateKernel(program, ref.allocCString(name).ref(), pErr));
    });
};
...
CLContext.prototype.createCommandQueue = function(device, properties) {
    return withCheck(function(pErr) {
        return new CLCommandQueue(opencl.clCreateCommandQueue(this.context, device.device, properties || null, pErr));
    });
};
...
CLContext.prototype.createBuffer = function(flags, size, hostPointer) {
    return withCheck(function(pErr) {
        return opencl.clCreateBuffer(this.context, flags, size, hostPointer || null, pErr)
    })
};
