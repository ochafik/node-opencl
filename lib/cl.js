...
CLCommandQueue.prototype.enqueueReadBuffer = function(buffer, blocking, offset, size, output, eventWaitList) {
    ...
    if (size > output.length) {
        throw 'Output buffer too small (buffer length = ' + output.length + ', read size = ' + size + ')';
    }
    return new CLEvent(withEvents(eventWaitList, function(numEventInWaitList, pEventWaitList, pEvent) {
        return opencl.clEnqueueReadBuffer(commandQueue, buffer.mem, blocking, offset, size, output, numEventInWaitList, pEventWaitList, pEvent);
    }, 'enqueueReadBuffer'));
};
...
CLCommandQueue.prototype.enqueueWriteBuffer = function(buffer, blocking, offset, size, input, eventWaitList) {
    ...
    if (size > input.length) {
        throw 'Input buffer too small (buffer length = ' + input.length + ', write size = ' + size + ')';
    }
    return new CLEvent(withEvents(eventWaitList, function(numEventInWaitList, pEventWaitList, pEvent) {
        return opencl.clEnqueueReadBuffer(commandQueue, buffer.mem, blocking, offset, size, input, numEventInWaitList, pEventWaitList, pEvent);
    }, 'enqueueWriteBuffer'));
};
...
CLKernel.prototype.setArg = function(i, value) {
    if (!value.mem) {
        throw 'Kernel value not handled yet';
    }
    check(opencl.clSetKernelArg(this.kernel, i, ref.types.size_t.size, value.mem.ref()), 'setArg');
};
